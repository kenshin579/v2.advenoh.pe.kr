...
### Phase 0: 선결 조건
1. ✅ **완료** — `supabase/migrations/*.sql` RLS 확인: `services` / `service_status_logs` / `daily_status_summary` 모두 anon read 허용(`TO anon, authenticated` 또는 `USING (true)`). `NEXT_PUBLIC_SUPABASE_ANON_KEY` 단독으로 조회 가능, service_key 불필요.
...
4. Netlify에 `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` 주입 (모든 컨텍스트)
...
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | 모든 컨텍스트 | Status 조회 (RLS anon read 허용 확인 완료) |
